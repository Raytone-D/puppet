...
2017/3/28 更新至v0.4.4，支持buy()/sell()直接输入数字，无需字符串。
...
* 支持同花顺的“所有”交易客户端：官方统一版或老版、券商定制版（银河、国泰君安、华泰、广发、东方财富等）。

* 支持多种交易模式：同一券商多个账户、多个券商多个账户同时进行交易。

* 仅支持多个不同的交易客户端登录后同时交易，一个交易客户端通过添加模式登录多个账户登录只能交易当前的那一个账户。
